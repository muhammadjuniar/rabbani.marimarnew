									...
									<div class="col-10 col-lg-10 m-auto p-0">
										<div class="user-card align-items-start">
											<div class="user-avatar bg-transparent md br-0">
												<router-link :to="{name: '2024.home'}"><img class="br-0" src="@/assets/images/empty.png"/></router-link>
											</div>
											<div class="user-info">
										...
										<div v-if="load.status == 'success' && member">
											...
											<div class="vote-count-content">
												...
												<div class="price-vote p-2 text-center mb-1 lh-150">
													<div class="title main-text-secondary">1 vote seharga Rp 1.250*</div>
													<div class="hide_element d-none sub-title main-color-secondary fs-9px fw-normal"></div>
												</div>
												<div class="product-vote mb-3">
																...
																<div v-for="(product, idxprd) in products" class="product-item ml-1 mr-1">
																	<div class="br-8" v-bind:class="{'is-selected' : product.is_selected}" >
																		<a @click.prevent="selectedProduct(idxprd)">
																			<div class="user-image product-image">
																				<div class="br-8 bg-light mb-0 w-100 h-100px box-image" :style="{ 'background-image': `url('${product.pic}') !important` }">
																				</div>
																			</div>
																			<div class="product-info text-center p-1">
																				<div class="main-color-secondary mb-1 fs-10px">{{product.title}}</div>
																				...
																				<div class="main-color-pink fs-9px">{{`Point Vote ${product.point}`}}</div>
																			</div>
																		</a>
																	</div>
																		...
																		<div v-else>
																			<div v-if="load_history.status == 'success' && histories && histories.length" class="text-left">
																				<div v-for="his in histories" class="item-history-vote fs-11px mb-2 main-text-secondary">
																					<span class="history-date">{{ `${dateFormat(his.paid_at, 'DD/MM/YYYY')} :` }}</span>
																					<span class="history-no ml-1">{{his.customer_phone_number}}</span>
																					<span class="history-count ml-1 pull-right">(<b>{{ `${his.total_votes}` }}</b>) <span class="fs-9px">vote</span></span>
																				</div>
														...
														<div class="col-12 col-lg-12 p-0 m-auto">
															<div class="card card-bordered mt-2">
																<div class="card-inner p-0">
																	<div class="card-title-group align-center m-0 mb-0 border-bottom p-2">
																		<div class="card-title card-title-sm">
																			<div class="overline-title fs-11px main-color-secondary-dark">Dukungan Lainnya</div>
																		</div>
																	</div>
																	<div class="card-body p-2 mh-300 h-auto content-overflow-y">
																		<div v-if="load_history_custom.status == 'pending'">
																			<LazyContent v-for="index in 5" class="mb-1" :isHeight="'20'" :isWidth="'w-100'" :isRadius="'br-4'"></LazyContent>
																		</div>
																		<div v-else>
																			<div v-if="load_history_custom.status == 'success' && histories_custom && histories_custom.length" class="text-left">
																				<div v-for="his_custom in histories_custom" class="item-history-vote fs-11px mb-2 main-text-secondary">
																					<span class="history-date">{{ `${dateFormat(his_custom.created_at, 'DD/MM/YYYY')} :` }}</span>
																					<span class="history-no ml-1 text-capitalize">{{his_custom.type}}</span>
																					<span class="history-count ml-1 pull-right">(<b>{{ (his_custom.status == 2) ? `-${his_custom.count}` : `${his_custom.count}` }}</b>) <span class="fs-9px">vote</span></span>
																					<div v-if="his_custom.description" class="main-text-secondary fs-10px pl-1 pt-1 fw-normal text-capitalize"><i>{{his_custom.description}}</i></div>
																				</div>
																				<div v-if="paginationDataHistory_custom.nextPage && paginationDataHistory_custom.nextPage > 0" class="text-center pt-1">
																					<a @click.prevent="handlePaginationHistoryCustom()" :disabled="(pagination_history_custom.load) ? true : false" v-bind:class="{disabled: pagination_history_custom.load}" class="btn btn-sm btn-white br-8 main-bg-light border-0 text-white">
																						<em :class="[!pagination_history_custom.load ? 'ni-chevrons-down' : 'ni-loader']" class="icon ni"></em>
																						<span>{{ (!pagination_history_custom.load) ? `Lebih Banyak (${paginationDataHistory_custom.currentPage}/${paginationDataHistory_custom.countPage})` : 'Sedang memuat...' }}</span>
																					</a>
																				</div>
																			</div>
																			<div v-else class="text-center">
																				<p class="p-0 m-0 text-soft fs-10px fw-normal"><i>{{ `"${load_history_custom.msg}"` }}</i></p>
																			</div>
																		</div>
																	</div>
																</div>
...
<script>
	...
	import { sleep, numberFormat, confirmAlert, dateFormat, initSlickSlider, loadingAlert, resAlert, textAlert } from '@/helpers/general'
	...
	import { checkVoteTransaction, postVoteTransaction, getHistoryVote, paymentChannelList, getHistoryCustomVote } from '@/controllers/transaction'
	...
	export default {
		setup(){
			...
			async function processVote(){
				...
				const result = await postVoteTransaction(sendVote, (parseInt(vote.product) > 0) ?true:false);
				if(result){
			...
			async function calculationSendVote(){
				...
				var sendVote = {
					total_votes:parseInt(vote.me), 
					registration_number:slugMember, 
					payment_method_code:vote.payment.code, 
					phone_number:vote.phone, 
					name:vote.name, 
			...
			async function handleQtyProduct(type, index){
				...
				if(type == 'plus'){
					if(currentQty < thisProduct.stock){
						currentQty += 1;
					}
				}else{
					if(currentQty > 0){
			...
			return { 
				...
				handleQtyProduct, destinations, load_destination, loadDestination, handleSelectDestination, shipments, handleSelectShipment
			}
		},
