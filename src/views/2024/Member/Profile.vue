													...
													<!-- <div class="user-card align-items-center mb-4">
														<div class="w-30">
															<div class="content-custom-select-image is-relative d-flex justify-content-center">
																<div @click.prevent="onSelectImage('foto-profile')" class=" xl bg-white main-border-secondary box-image">
													...
													<div class="content-profile">
														...
														<div class="body-content-profile mb-3">
															<div class="card card-daftar-member">
																<div class="row align-items-center mb-3">
																	...
																	<div class="col-8">
																		<div class="foto-member mb-2">Foto Member</div>
																		<div class="btn btn-sm btn-upload" @click.prevent="onSelectImage('foto-profile')">Upload</div>
																	</div>
																		</div>
																	</div>
																	...
																	<div class="col-lg-12">
																		...
																		<div class="form-group mb-3">
																			<label class="form-label-profile form-label-profile form-label mb-1 fs-12px main-color-secondary-marimar w-100">Nomor Whatsapp <span class="main-color-secondary-marimar fs-9px lh-normal fw-normal">wajib diisi</span></label>
																			<input minlength="8" maxlength="15" v-on:keypress="numbersOnly" :disabled="(new_member.status_member == 'confirm') ? true : false" v-bind:class="{ disabled: new_member.status_member == 'confirm' }" type="text" v-model="new_member.phone_number"  class="form-control form-control-lg br-8 pl-3 pr-3 pt-4 pb-4 border-0 main-bg-cream-light main-text-secondary-marimar text-capitalize" placeholder="08XXXXXXXXXX">
																		</div>
														...
														<div class="nk-block mt-5 text-center">
															<button type="submit"
																...
																>{{ (new_member.status_member == 'confirm') ? 'KONFIRMASI' : 'TAMBAH MEMBER' }}</button
															>
														</div>
													</form>	
...
<script>
	...
	export default {
		setup(){
			...
			const member_code = (session.getters.stateUser && session.getters.stateUser.member_code) ? session.getters.stateUser.member_code : null;
			...
			const processRegisterMember = async () => {
				...
				if(new_member.status_member == 'confirm'){
					...
					const result = await postMember({phone_number:new_member.phone_number, member_group:profile.name, member_id:member_id});
					if(result){
						new_member.status_member = 'confirm';
					}
				}
			}
			const processConfirmMember = async () => {
				var existPic = await getFileUpload('file-select-image-foto-profile');
			...
			return {
				procesLogout, user, new_member, profile, birthdate, load, numbersOnly, socialList, talents,
				getAssetFile, onChangeLocation, achievements, actionAchievement, handleValueAchievement,
				onChangeSelectImage, onSelectImage, onResetImage, processRegisterMember, processUpdateProfile, profilePic,
