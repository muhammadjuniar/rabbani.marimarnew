							...
							<div class="card-inner p-0">
								<div v-if="load.status == 'pending'">
									<div class="nk-block mb-5 p-2 mt-5">
										<div class="row align-items-center justify-content-between m-0">
											...
											<div class="title-logo"><div class="user-avatar md bg-light"></div></div>
										</div>
									</div>
									<div class="nk-block p-1 pl-2 pr-2">
										<LazyContent class="mb-4" :isHeight="'75'" :isWidth="'w-100'"></LazyContent>
								...
								<div v-cloak v-else v-bind:class="{ hide_element: load.status == 'pending' }">
									<div v-if="load.status == 'success'">
										<div id="member-area-top" class="nk-block mb-2 p-4 pt-5" :style="{ 'background-image': `url('${getAssetFile('images','img-bg-profile.png')}')` }">
											<div class="row align-items-center justify-content-between m-0 mb-1">
												<div class="title w-75">
													<h2 class="text-uppercase main-color ls-sm mb-2">Selamat Datang</h2>
													<div class="row m-0 align-items-center mb-1">
														<h4 class="text-uppercase main-color ls-sm mb-0 w-auto mw-80 text-short">{{ (user.member_name) ? user.member_name : ( (user.member_phone) ? user.member_phone : 'DPR Sekolah Dasar' ) }}</h4>
												...
												<div class="title-logo">
													<div class="user-avatar md bg-lighter box-image">
														<img class="br-0" :src="(profilePic) ? profilePic : getAssetFile('images','img-dprd-lg.png')"/>
													</div>
												</div>
											</div>
										</div>
											...
											<div class="col-11 col-lg-11 m-auto p-0">
												<form @submit.prevent="processUpdateProfile">
													...
													<div class="content-profile">
														<div class="head-content-profile mb-2 mt-4">
															<div class="fs-16px main-color-secondary fw-bold">Profile DPR / MPR</div>
														</div>
														<div class="body-content-profile mb-3">
															<div class="form-group mb-3">
																<label class="form-label-profile form-label-profile form-label mb-1 fs-12px main-color-secondary w-100">Nama Lengkap <span class="main-color-secondary fs-9px lh-normal fw-normal">wajib diisi</span></label>
																<input type="text" v-model="profile.name" required class="form-control form-control-lg br-8 pl-3 pr-3 pt-4 pb-4 border-0 main-bg-cream-light main-text-secondary text-capitalize" placeholder="isi nama lengkap mu disini">
															</div>
															<div class="form-group mb-3">
															...
															<div class="form-group mb-2">
																<label class="form-label-profile form-label mb-1 fs-12px main-color-secondary">Jenis Kelamin <span class="main-color-secondary fs-9px lh-normal fw-normal">wajib diisi</span></label>
																<select required class="form-control form-control-lg br-8 main-color-secondary-dark disabled main-bg-cream-light" v-model="profile.gender">
																	<option value="male">Laki-Laki</option>
																	<option value="female">Perempuan</option>
																</select>
															</div>
															<div class="mb-3 address-content">
																<div class="form-group mb-2">
																	<label class="form-label-profile form-label mb-1 fs-12px main-color-secondary">Alamat <span class="main-color-secondary fs-9px lh-normal fw-normal">wajib diisi</span></label>
																	<input type="text" v-model="profile.address" required class="form-control form-control-lg br-8 pl-3 pr-3 pt-4 pb-4 border-0 main-bg-cream-light main-text-secondary text-capitalize" placeholder="alamat lengkap">
																</div>
																<div class="form-group mb-2">
																	<div class="form-control-wrap select-custom">
																		<div class="form-icon form-icon-right"><em class="icon ni ni-chevron-down fs-13 text-secondary"></em></div>
																		<select @change.prevent="onChangeLocation($event, 'province')" required id="is-select-province" disabled class="form-control form-control-lg br-8 main-color-secondary-dark disabled main-bg-cream-light">
																			<option value="">Pilih provinsi</option>
																			<option v-if="location.province" :value="location.province">{{ user.province_name }}</option>
																		</select>
																	</div>
																</div>
																<div class="form-group mb-2">
																	<div class="form-control-wrap select-custom">
																		<div class="form-icon form-icon-right"><em class="icon ni ni-chevron-down fs-13 text-secondary"></em></div>
																		<select @change.prevent="onChangeLocation($event, 'city')" required id="is-select-city" disabled class="form-control form-control-lg br-8 main-color-secondary-dark disabled main-bg-cream-light">
																			<option value="">Pilih kota/kabupaten</option>
																			<option v-if="location.city" :value="location.city">{{ user.regency_namde }}</option>
																		</select>
																	</div>
																</div>
																<div class="form-group mb-2">
																	<div class="form-control-wrap select-custom">
																		<div class="form-icon form-icon-right"><em class="icon ni ni-chevron-down fs-13 text-secondary"></em></div>
																		<select required id="is-select-district" disabled class="form-control form-control-lg br-8 main-color-secondary-dark disabled main-bg-cream-light">
																			<option value="">Pilih kecamatan</option>
																			<option v-if="location.district" :value="location.district">{{ user.district_name }}</option>
																		</select>
																	</div>
																</div>
															</div>
															<div class="form-group mb-2">
																<label class="form-label-profile form-label mb-1 fs-12px main-color-secondary">Asal Sekolah <span class="main-color-secondary fs-9px lh-normal fw-normal">wajib diisi</span></label>
																<input type="text" v-model="profile.school" required class="form-control form-control-lg br-8 pl-3 pr-3 pt-4 pb-4 border-0 main-bg-cream-light main-text-secondary" placeholder="isi sekolah mu disini">
															</div>
															<div class="form-group mb-2">
																<label class="form-label-profile form-label mb-1 fs-12px main-color-secondary">No Whatsapp <span class="main-color-secondary fs-9px lh-normal fw-normal">wajib diisi</span></label>
																<input type="text" disabled minlength="8" maxlength="15" v-on:keypress="numbersOnly" required class="disabled form-control form-control-lg br-8 pl-3 pr-3 pt-4 pb-4 border-0 main-bg-cream-light main-text-secondary" :value="user.member_phone" placeholder="08xxxxxx">
															</div>
														</div>
														<div class="head-content-profile mb-2 mt-4">
															<div class="fs-16px main-color-secondary fw-bold mb-1">Prestasi</div>
															<p class="main-color-secondary fs-10px fw-normal">Isi berpengaruh terhadap penilaian</p>
														</div>
														<div class="body-content-profile mb-3">
															<div class="content-profile-prestasi mb-2">
																<div v-for="(prestasi, index) in achievements" class="form-group mb-2">
																	<label class="form-label-profile  form-label-profile form-label mb-1 fs-12px main-color-secondary">{{ `Prestasi ${index+1}` }} <span v-if="user.level_publish == 0" @click.prevent="actionAchievement('remove', index)" class="main-color fs-9px lh-normal">hapus</span></label>
																	<input @keyup.prevent="handleValueAchievement($event, index)" type="text" :value="prestasi" :id="`achievement-${index}`" required class="form-control form-control-lg br-8 pl-3 pr-3 pt-4 pb-4 border-0 main-bg-cream-light main-text-secondary text-capitalize" placeholder="Isi prestasi kamu disini">
																</div>
																<div class="text-center mt-2">
																	<a v-bind:class="{disabled: user.level_publish == 1}" :disabled="user.level_publish == 0" @click.prevent="actionAchievement('add')" class="btn btn-md fs-13px main-color-secondary btn-lighter">Tambah</a>
																</div>
															</div>
															<div class="form-group mb-3">
																<label class="form-label-profile form-label mb-1 fs-12px main-color-secondary">Bakat Kamu <span class="main-color-secondary fs-9px lh-normal fw-normal">wajib diisi</span></label>
																<div class="form-control-wrap select-custom">
																	<div class="form-icon form-icon-right"><em class="icon ni ni-chevron-down fs-13 text-secondary"></em></div>
																	<select v-model="profile.talent" required class="form-control form-control-lg br-8 main-color-secondary-dark disabled main-bg-cream-light">
																		<option value="">Pilih bakat kamu</option>
																		<option v-for="talent in talents" :value="talent.key">{{talent.text}}</option>
																	</select>
																</div>
															</div>
															<div class="form-group mb-3">
																<label class="form-label-profile form-label mb-1 fs-12px main-color-secondary">Toko Rabbani Terdekat <span class="main-color-secondary fs-9px lh-normal fw-normal">wajib diisi</span></label>
																<div class="form-control-wrap select-custom">
																	<div class="select-store-dpr">
																		<select required id="is-select-store" disabled class="disabled form-control form-control-lg br-8 main-color-secondary-dark disabled main-bg-cream-light">
														...
														<div class="head-content-profile mb-2 mt-4">
															<div class="fs-16px main-color-secondary fw-bold">Sosial Media</div>
														</div>
														<div class="body-content-profile mb-3">
															<div v-for="social in socialList" class="user-card align-items-start mb-2">
																<div class="user-avatar sm bg-transparent br-4 w-10">
																	<img class="br-4" :src="getAssetFile('icons', `${social}.png`)"/>
																</div>
																<div class="user-info w-90">
																	<div class="form-group mb-1">
																		<input v-model="profile[`${social}_username`]" type="text" class="form-control form-control-lg br-8 pl-3 pr-3 pt-4 pb-4 border-0 main-bg-cream-light main-text-secondary" placeholder="URL username">
																	</div>
																	<div class="form-group m-0">
																		<input v-model="profile[`${social}_followers`]" type="number" class="form-control form-control-lg br-8 pl-3 pr-3 pt-4 pb-4 border-0 main-bg-cream-light main-text-secondary" placeholder="Jumlah followers">
																	</div>
																</div>
															</div>
														</div>
														...
														<div class="head-content-profile mb-2 mt-4">
															<div class="fs-16px main-color-secondary fw-bold mb-1">Galeri</div>
															<p class="main-color-secondary fs-10px fw-normal">Upload lebih banyak foto</p>
														</div>
														<div class="body-content-profile mb-3">
															<div class="row">
																<div class="col-6 col-lg-6 mb-2" v-for="(foto, fotoidx) in fotos.data" :key="fotoidx">
																	<div class="content-custom-select-image is-relative d-flex justify-content-center bg-white main-border-secondary br-8">
																		<div @click.prevent="onSelectImage(`foto-profile-${fotoidx}`)" class="user-avatar xl box-image br-4 bg-transparent">
																			<img :default="(foto.pic) ? foto.pic : getAssetFile('images','empty.png')" class="br-0" :id="`preview-select-image-foto-profile-${fotoidx}`" :src="(foto.pic) ? foto.pic : getAssetFile('images','empty.png')"/>
																		</div>
																		<input :id="`file-select-image-foto-profile-${fotoidx}`" type="file" @change.prevent="onChangeSelectImage($event, `foto-profile-${fotoidx}`)" class="custom-file-input file-select-image is-absolute w-100 h-100 br-80" name="foto">
																		<div :id="`action-reset-select-image-foto-profile-${fotoidx}`" class="action-reset-select-image is-auto none-select top-3 right-0 left-3 is-absolute bg-transparent">
																			<a @click.prevent="onResetImage(`foto-profile-${fotoidx}`)" disabled class="reset-select-image disabled btn btn-icon btn-danger btn-dim main-border-secondary"><em class="icon ni ni-reload"></em></a>
																		</div>
																	</div>
																	<div v-if="user.level_publish == 0" class="action-for-main-foto-profile p-1 br-4 mt-0">
																		<div class="row align-items-center justify-content-center m-0">
																			<a @click.prevent="actionFotos('update', fotoidx)" class="btn btn-block btn-sm main-bg text-white">Simpan</a>
																		</div>
																	</div>
																</div>
																<div v-if="user.level_publish == 0" class="col-6 col-lg-6 mb-2">
																	<div class="content-custom-select-image is-relative d-flex justify-content-center">
																		<div class="user-avatar xl bg-white main-border-secondary box-image br-8">
																			<a @click.prevent="actionFotos('add')" class="btn btn-md fs-13px main-color-secondary btn-white"><em class="icon ni ni-plus"></em><span class="pl-1">Tambah</span></a>
																		</div>
																	</div>
																</div>
															</div>
